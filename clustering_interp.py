...
from dec import ClusterAssignment, target_distribution 
...
class Net(nn.Module):
    def __init__(self, args, device):
        ...
        self.device = device
        self.args = args
        self.num_variables = args.num_variables    #number of observable variables
        self.num_timestamps = args.num_timestamps  #max number of time stamps
        self.nhidden = 128
        self.nlstm = 1
        ...
        self.encoder = EncoderRNN(input_size=3 * self.num_variables, hidden_size=self.nhidden, 
        ...
        ###clustering    
        alpha = 1.0
        self.cluster_assignment = ClusterAssignment(self.args.cluster_number, self.dim_enc_hidden, alpha)        
    ...
    def forward(self, x, fake_x=None, fake_perm_idx=None, positive_x=None):
        ...
        ###context: time_step, batch_size, hidden_size * (2 if bidrection else 1), last lstm layer output
        ...
        context, hidden, cell_state = self.encoder(interp_x)  # LSTM
        
        ###concate the hidden output, shape(bath_size, num_layers *  (2 if bidrection else 1) * hidden_size)
        cat_hidden = torch.cat([x for x in hidden], dim=-1) 
        y, _ = self.decoder(context, hidden, cell_state)
        y = y.permute(1, 2, 0)  # (batch, hidden, ref_t)
        ...
        if self.args.triple_margin != 0. and self.args.fake_detection:
            pos_interp_x = self.sci(positive_x)
            pos_interp_x = self.cci(pos_interp_x)  # (batch, time_step, channel)

            pos_interp_x = pos_interp_x.permute(1, 0, 2)  # (time_step, batchsize, channel)
            pos_context, pos_hidden, pos_cell_state = self.encoder(pos_interp_x)  # LSTM
            pos_cat_hidden = torch.cat([x for x in pos_hidden], dim=-1)

            aux_pred_dict['positive'] = pos_cat_hidden
            aux_pred_dict['negative'] = fake_cat_hidden

        cluster_pred = self.cluster_assignment(cat_hidden)
        cluster_label = target_distribution(cluster_pred).detach()
        aux_pred_dict['cluster_pred'] = cluster_pred
        aux_pred_dict['cluster_label'] = cluster_label
        return cat_hidden, y, aux_pred_dict    
        
    def init_cluster_center(self, initial_cluster_centers):
        self.cluster_assignment.init_center(initial_cluster_centers)

    def get_cluster_center(self):
        return self.cluster_assignment.get_center()
    ...
    def kl_loss(self, label, pred):
        kl_div = F.kl_div(pred.log(), label, reduction='batchmean')
        return {'kl': kl_div}
    ...
    def triplet_loss(self, anchor, positive, negative, margin):
        triple = F.triplet_margin_loss(anchor, positive, negative, margin=margin, reduction='mean')
        return {'triplet': triple}
